...
function adjust_CO2_capture(CC_share)
    for t in keys(techCC)
        OutputRatio[t,"CO2"] = round(CC_share*EmissionRatio[techCC[t]]; digits = 2)
...
EmissionRatio, OutputRatio = adjust_CO2_capture(0.9)
...
@constraint(ESM, NoCO2Curtailment[y in year, r in regions,h in hour],
    Curtailment[y,r,h,"CO2"] == 0
)
...
@constraint(ESM, ProductionFuntion_disp[y in year,r in regions,h in hour, t in technologies, f in fuels;TagDispatchableTechnology[t]>0 && f!="CO2"],
...
@constraint(ESM, ProductionFunction_cc[y in year, r in regions, h in hour, t in technologies; TagCCTechnology[t]>0], 
    Production[y,r,h,t,"CO2"] == OutputRatio[t,"CO2"] * (Production[y,r,h,t,"Power"] + Production[y,r,h,t,"Heat"])
)

# for variable renewables, the production needs to be always at maximum
@constraint(ESM, ProductionFunction_res[y in year,r in regions,h in hour, t in technologies, f in fuels; TagDispatchableTechnology[t]==0 && f!="CO2"], 
    OutputRatio[t,f] * TotalCapacity[y,r,t] * CapacityFactor[r,t,h] == Production[y,r,h,t,f]
)
...
@constraint(ESM, StorageLevelStartFunction[y in year,r in regions,s in storages, h in hour, f in fuels; h==1 && StorageDisChargeEfficiency[s,f]>0 && s!="CO2Perm"], 
...
# NEW FOR CO2Perm: storage level for first year is set to 0
@constraint(ESM, StorageLevelYear0C02PermFunction[y in year,r in regions,s in storages, h in hour, f in fuels; h==1 && y==first_year && StorageDisChargeEfficiency[s,f]>0 && s=="CO2Perm"], 
    StorageLevel[y,r,s,h,f] == 0
)

# NEW FOR CO2Perm: storage level for next year is equal to the last result from the previous year
@constraint(ESM, StorageLevelStartCO2PermFunction[y in year,r in regions,s in storages, h in hour, f in fuels; h==1 && y!=first_year && StorageDisChargeEfficiency[s,f]>0 && s=="CO2Perm"], 
    StorageLevel[y,r,s,h,f] == StorageLevel[y-10,r,s,n_hour,f]
)
