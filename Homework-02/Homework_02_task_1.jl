...
@constraint(ESM, StorageLevelFunction[s in storages, h in hour, f in fuels; h>1 && StorageDisChargeEfficiency[s,f]>0], StorageLevel[s,h,f] == StorageLevel[s,h-1,f] + StorageCharge[s,h,f]*StorageChargeEfficiency[s,f] - StorageDischarge[s,h,f]/StorageDisChargeEfficiency[s,f])
...
@constraint(ESM, StorageAnnualBalanceFunction[s in storages, f in fuels; StorageDisChargeEfficiency[s,f]>0], sum(StorageCharge[s,h,f] for h in hour)*StorageChargeEfficiency[s,f] - sum(StorageDischarge[s,h,f] for h in hour) / StorageDisChargeEfficiency[s,f]  == 0)
...
@constraint(ESM, MaxStorageCapacityFunction[s in storages], sum(StorageEnergyCapacity[s,f] for f in fuels if StorageDisChargeEfficiency[s,f]>0) <= MaxStorageCapacity[s])
